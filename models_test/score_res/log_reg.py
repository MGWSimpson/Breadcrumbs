...
def train_logistic_regression():
    ...
    visualize_decision_boundaries(X_test_scaled, y_test, model, scaler)
    
    print("\nLogistic regression coefficients:")
    for i, class_name in enumerate(model.classes_):
        print(f"Class: {class_name}")
        print(f"  Coefficient for score1: {model.coef_[i][0]:.4f}")
        print(f"  Coefficient for score2: {model.coef_[i][1]:.4f}")
        print(f"  Intercept: {model.intercept_[i]:.4f}")
        print()
    
    return model, scaler

def visualize_decision_boundaries(X, y, model, scaler, plot_step=0.02):
...
def visualize_data_distribution(X, y):
    ...
    color_map = {'Human': 'blue', 'Raw AI': 'red', 'Rephrased AI': 'green'}
...
def main():
    print("Training logistic regression on data from JSON...")
